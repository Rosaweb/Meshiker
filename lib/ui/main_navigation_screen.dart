...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  ...
  Widget build(BuildContext context) {
    return Consumer<SettingsService>(
      builder: (context, settings, _) {
          ...
          appBar: AppBar(
            title: Row(
              children: [
                ...
                const Text('Rando Offline', style: TextStyle(fontWeight: FontWeight.bold)),
              ],
            ),
            backgroundColor: Colors.black,
          ...
          body: ListView(
            children: [
              ...
              _buildSettingsTile(
                ...
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const UnitsSettingsScreen())),
              ),
              _buildSettingsTile(
                icon: Icons.map_outlined,
                title: 'Mes cartes',
                subtitle: 'Sélectionner vos fonds de carte favoris',
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MapsSettingsScreen())),
              ),
              _buildSettingsTile(
                icon: Icons.route_outlined,
                title: 'Track Manager',
                subtitle: 'Gérer vos pistes GPX',
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const TrackManagerScreen())),
              ),
              _buildSettingsTile(
                icon: Icons.location_on_outlined,
                title: 'Waypoint Manager',
                subtitle: 'Gérer, filtrer et classer vos points',
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const WaypointManagerScreen())),
              ),
            ],
          ),
        );
          ...
          child: Scaffold(
            ...
            body: ListView(
              ...
              children: [
                ...
                const Text('PROCHAIN WAYPOINT', style: TextStyle(color: Colors.greenAccent, fontSize: 12, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                ValueListenableBuilder<Waypoint?>(
                  valueListenable: widget.recordingService.nextWaypoint,
                  builder: (context, wp, _) {
                    if (wp == null) return const Text('Aucun waypoint à venir', style: TextStyle(color: Colors.white38));
                    return ValueListenableBuilder<double>(
                      valueListenable: widget.recordingService.distanceToNextWaypointMeters,
                      builder: (context, dist, _) {
                        final avgGlobal = widget.recordingService.averageSpeedGlobalMps.value;
                        final eta = (avgGlobal > 0.5) ? _formatDuration(dist / avgGlobal) : '--:--';
                ...
                const SizedBox(height: 24),
                const Text('DESTINATION', style: TextStyle(color: Colors.blueAccent, fontSize: 12, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                ValueListenableBuilder<Waypoint?>(
                  valueListenable: widget.recordingService.destinationWaypoint,
                  builder: (context, wp, _) {
                    if (wp == null) return const Text('Aucune destination choisie', style: TextStyle(color: Colors.white38));
                    return ValueListenableBuilder<double>(
                      valueListenable: widget.recordingService.distanceToDestinationMeters,
                      builder: (context, dist, _) {
                        final avgGlobal = widget.recordingService.averageSpeedGlobalMps.value;
                        final eta = (avgGlobal > 0.5) ? _formatDuration(dist / avgGlobal) : '--:--';
                        return _buildNavigationInfo(
                ...
                const Divider(color: Colors.white24, height: 40),
                SwitchListTile(
                  title: const Text('Afficher les Waypoints sur la carte', style: TextStyle(color: Colors.white, fontSize: 14)),
        ...
        final wpManagerPage = const WaypointManagerScreen();
        ...
        final List<Widget> pages = settings.reversePanels 
            ? [
                wpManagerPage,
                contextualPage, 
                const IgnorePointer(child: SizedBox.expand()), // Trou pour la carte à l'index 2
                settingsPage
              ]
            : [
                settingsPage, 
                const IgnorePointer(child: SizedBox.expand()), // Trou pour la carte à l'index 1
                contextualPage,
                wpManagerPage
              ];
          ...
          body: Stack(
            children: [
              ...
              PageView(
                ...
                physics: const NeverScrollableScrollPhysics(), // Désactive le swipe natif
                children: pages,
              ),

              // 3. CAPTURE DU SWIPE SUR LES BORDS (40px)
              // Bande Gauche
              Positioned(
  ...
  void _snapPageView() {
    final page = _pageController.offset / MediaQuery.of(context).size.width;
  ...
  Widget _buildLiveStatsGrid() {
    ...
    return GridView.count(
      ...
      children: [
        AnimatedBuilder(
          animation: Listenable.merge([recording.currentSpeedMps, recording.averageSpeedDailyMps, recording.averageSpeedGlobalMps]),
          builder: (context, _) => _buildSpeedCard(),
        ),
        ValueListenableBuilder<double>(
          valueListenable: recording.dailyDistanceMeters,
          builder: (context, dist, _) => _buildStatCard('Aujourd\'hui', _formatDistance(dist), Icons.today),
        ),
        ValueListenableBuilder<double>(
          valueListenable: recording.gpsAccuracyMeters,
          builder: (context, acc, _) => _buildStatCard('Précision GPS', '${acc.round()} m', Icons.satellite_alt),
        ),
        ValueListenableBuilder<double>(
          ...
          builder: (context, done, _) {
            return ValueListenableBuilder<double>(
              ...
              builder: (context, remaining, _) {
                ...
                return _buildStatCard('Trace', '${_formatDistance(done)} / ${_formatDistance(total)}', Icons.route);
              }
            );
          },
        ),
        AnimatedBuilder(
          ...
          builder: (context, _) => _buildStatCard('Pas', '${widget.pedometerService.steps}', Icons.directions_walk, isActive: widget.pedometerService.isActive, onTap: () => widget.pedometerService.togglePedometer()),
        ),
        ValueListenableBuilder<geo.Position?>(
          valueListenable: widget.recordingService.currentPosition,
          builder: (context, pos, _) => _buildStatCard('Satellites', pos != null ? 'FIX OK' : 'RECHERCHE', Icons.satellite_alt),
        ),
      ],
    );
  }
  ...
  Widget _buildSpeedCard() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [const Icon(Icons.speed, color: Colors.greenAccent, size: 14), const SizedBox(width: 4), const Text('Vitesse', style: TextStyle(color: Colors.white38, fontSize: 10))]),
          const Spacer(),
          Center(child: Text(current, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))),
          const Spacer(),
          Text('Moy. jour: $avgDaily', style: const TextStyle(color: Colors.white70, fontSize: 8)),
          Text('Moy. gén.: $avgGlobal', style: const TextStyle(color: Colors.white70, fontSize: 8)),
        ],
      ),
    );
  }

  Widget _buildMeasureButton(BuildContext context, {required String label, required IconData icon, required VoidCallback onPressed}) {
  ...
  Widget _buildStatCard(String label, String value, IconData icon, {bool isActive = false, VoidCallback? onTap}) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        padding: const EdgeInsets.all(8),
        decoration: BoxDecoration(color: Colors.white.withOpacity(0.05), borderRadius: BorderRadius.circular(12), border: Border.all(color: isActive ? Colors.greenAccent : Colors.white10, width: isActive ? 2.0 : 1.0)),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            FittedBox(fit: BoxFit.scaleDown, child: Text(value, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))),
          ],
        ),
      ),
    );
  ...
  String _formatSpeed(double mps) => widget.settingsService.unitSystem == UnitSystem.metric ? '${(mps * 3.6).toStringAsFixed(1)} km/h' : '${(mps * 2.23694).toStringAsFixed(1)} mph';
  ...
  Widget _buildNavigationInfo({required String name, required String type, required String distance, required String eta, required bool isNext}) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white.withOpacity(0.05), borderRadius: BorderRadius.circular(8), border: Border.all(color: isNext ? Colors.greenAccent.withOpacity(0.2) : Colors.blueAccent.withOpacity(0.2))),
      child: Row(
        children: [
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              const Text('Bienvenue !', style: TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.bold)),
              const SizedBox(height: 40),
              Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [_HintGesture(icon: Icons.arrow_back, text: 'Swipe vers la droite\n$leftLabel'), _HintGesture(icon: Icons.arrow_forward, text: 'Swipe vers la gauche\n$rightLabel')]),
              const SizedBox(height: 60),
              const Text('Appuyez pour commencer', style: TextStyle(color: Colors.white70)),
            ],
          ),
        ),
      ),
...
class _HintGesture extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Column(children: [Icon(icon, color: Colors.blueAccent, size: 40), const SizedBox(height: 8), Text(text, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white))]);
}
